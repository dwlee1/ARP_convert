...
def scan_all(scan_dir, verbose=False):
    ...
    if zstd_skipped > 0:
        ...
        print("    -> pip install zstandard 설치 후 재실행하세요")
...
def print_summary(results):
    ...
    for folder in sorted(folder_stats.keys()):
        ...
        print(
            f"    {folder:20s} {s['total']:5d} {s['arp']:5d} {s['custom']:6d} {s['none']:5d} {pct:5.1f}%"
        )
...
def main():
    ...
    if not _HAS_ZSTD:
        print("  [!] zstandard 미설치 -- zstd 압축 파일은 스킵됩니다")
        print("    설치: pip install zstandard")
    print()
