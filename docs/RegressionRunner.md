...
> GUI Blender 세션용 개발/회귀 테스트 문서

## 먼저 알아둘 점

이 러너는 대량 동물 처리 자동화 도구가 아니다.

- 이 러너의 목적은 대표 샘플 몇 개를 고정해 두고 코드 수정 후 회귀 여부를 빠르게 확인하는 것이다
- 모든 동물마다 fixture JSON을 만드는 구조를 목표로 하지 않는다
- 실제 자동화의 핵심은 `scripts/skeleton_analyzer.py`가 새 동물에서도 역할을 얼마나 잘 자동 추론하느냐에 있다
- 따라서 이 문서는 "개발용 회귀 테스트" 문서로 읽어야 한다
...
- 실제 변환 자동화:
  - 새 동물을 넣었을 때 구조 분석이 역할을 자동 추론하고 BuildRig까지 안정적으로 가는 것
- 회귀 테스트 자동화:
  - 대표 샘플 몇 개를 기준으로 최근 코드 수정이 기존 성공 케이스를 깨지 않았는지 확인하는 것

현재 이 문서의 러너는 두 번째 범위에 속한다.
...
- 모든 동물마다 fixture를 만들지 않는다
- 대표 샘플 `.blend` 몇 개와 fixture를 짝으로 유지한다
- 예시:
  - 일반 사족보행 1종
  - toe 없는 케이스 1종
  - ear 체인이 긴 케이스 1종
  - bank / heel 보정이 중요한 케이스 1종
- F7, F6 같은 리그 생성 규칙 변경 후 같은 fixture를 다시 돌려 회귀 여부를 본다
- 결과 확인은 JSON 리포트와 Blender 결과를 함께 본다

## 실제 자동화 우선순위

이 러너를 늘리는 것보다 우선해야 할 것은 아래다.

1. `scripts/skeleton_analyzer.py`의 역할 자동 추론 정확도 개선
2. low-confidence 케이스만 사람이 수정하도록 흐름 단순화
3. 대표 샘플 회귀 테스트로 기존 성공 케이스 보호

즉, fixture 수를 늘리는 것이 아니라 자동 추론 성공률을 높이는 쪽이 본 프로젝트의 주된 자동화 방향이다.
