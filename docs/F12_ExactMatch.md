...
| constraint 타이밍 | 전체 본에 한 번에 추가 → `nla.bake()` 1회 → 전체 제거 | 성능 + 동일 시점 평가 보장 |
| 본 선택 | `bone_pairs` 목록 기준으로 컨트롤러만 선택 | 불필요한 본에 키프레임 생성 방지 |
...
> **이전 설계에서 "양쪽 오브젝트 transform (0,0,0) 권장"이었으나, WORLD→WORLD 복사가 오브젝트 원점 어긋남에 민감하므로 hard check로 승격함.**
...
Bake Animation (신규, 별도 버튼)
  ...
  ↓ 소스의 모든 액션 순회 (각 액션마다):
      ...
      4. 전체 bone_pairs의 ARP 컨트롤러에 COPY_TRANSFORMS (WORLD→WORLD) 추가
         (constraint 이름: "ARPCONV_CopyTF")
      5. bone_pairs의 컨트롤러 본만 선택
      6. nla.bake(visual_keying=True, clear_constraints=False)
      7. "ARPCONV_CopyTF" constraint만 제거
      8. 역할 본(is_custom=false)의 Scale FCurve 삭제
      9. ARP에 새 액션 생성 (원본명 + "_arp")
      10. 임시 NLA strip 제거, 뮤트 복원
  ↓ 결과: ARP 컨트롤러에 소스와 동일한 월드 공간 모션
```
...
```python
try:
    # constraint 추가 → bake → constraint 제거
finally:
    # 모든 본에서 "ARPCONV_CopyTF" constraint 제거
    # 절반만 구워진 액션 삭제
    # NLA mute 상태 복원
```
...
| 파일 | 수정 내용 |
|------|-----------|
| `scripts/arp_utils.py` | `bake_with_copy_transforms()`, `bake_all_actions()` 추가 |
...
def bake_with_copy_transforms(source_obj, arp_obj, bone_pairs, frame_start, frame_end):
    ...
    동작:
        1. 전체 bone_pairs에 COPY_TRANSFORMS 추가 (이름: "ARPCONV_CopyTF")
        2. bone_pairs의 컨트롤러 본만 선택
        3. nla.bake(visual_keying=True, clear_constraints=False)
