...
> 최종 수정: 2026-03-27 (F9 Remap UI 통합 추가)
...
9. Shape key 드라이버 리맵
10. Remap 설정 (Auto Map → UI 리스트에서 확인/수정)   ← F9
11. 액션별 retarget (FK 또는 IK 모드)
```
...
- [x] F1: 웨이트 0 본 프리뷰 제외 (pytest + Blender 테스트 완료)
- [x] F2: IK pole vector 위치 매칭 (Fox/Cat Blender 테스트 완료)
- [x] F3: Shape key 드라이버 보존 (Cat Blender 테스트 완료)
- [ ] F4: 리타게팅 IK 모드 — 코드 완료, Blender 테스트 대기 (Blender 4.5 Action Slot 호환성 수정 완료)
- [ ] F9: Remap UI 통합 — 설계 완료, 구현 대기 (FeaturePlan_v5.md)
- [x] 개발 하네스: pyproject.toml, pre-commit (ruff + ruff-format), pytest 연동
...
| `scripts/arp_convert_addon.py` | Preview UI, BuildRig, Remap UI, Retarget |
...
### F9. Remap UI 통합 (본 매핑 편집)

#### 문제

- 리타게팅 매핑이 사용자에게 보이지 않음 — `generate_bmap_content()`가 자동 생성하지만 확인/수정 불가
- 체인 수 불일치로 폐기된 본이 어떤 것인지 UI에서 알 수 없음
- ARP Remap 패널은 내부 상태 비공개로 직접 통합 불가

#### 목표

- 자체 `CollectionProperty` + `UIList`로 Source ↔ Target 매핑 리스트 표시
- BuildRig 후 자동 채움 (Auto Map) → 사용자 확인/수정 → .bmap 변환 → 리타게팅
- 매핑된 본, 폐기된 본, 커스텀 본을 구분 표시

#### 상세 설계

- `docs/FeaturePlan_v5.md` 참조

#### 변경 파일

| 파일 | 변경 |
|------|------|
| `scripts/arp_convert_addon.py` | `ARPCONV_BoneMapEntry` PropertyGroup, `ARPCONV_UL_BoneMap` UIList, `ARPCONV_OT_AutoMap` 오퍼레이터, 패널 Step 3.5 추가, Retarget 수정 |
| `scripts/skeleton_analyzer.py` | `populate_bone_map()` 함수 추가 (기존 `generate_bmap_content` 로직 재사용) |
...
1. **F4 (FeaturePlan_v4): 리타게팅 IK 모드 Blender 테스트** — 코드 완료, 검증 대기
2. **F1: 체인 매칭 ARP 네이티브 방식 교체** — 현재 구현이 동작하지 않으므로 최우선
3. **F8: 웨이트 전송 실제 검증** — 구현 완료, 여우 리그 weight paint 검증 필요
4. **F9: Remap UI 통합** — 매핑 확인/수정 UI (FeaturePlan_v5.md)
5. F7: Remap `.bmap` 규칙 정리 — 컨트롤러 이름 직접 탐색 방식 연동 (F9와 연계)
6. F6: bank / heel 자동 배치 보정
7. F4 (ProjectPlan): 소스 컨스트레인트 복제
...
- **Blender 4.5 Action Slot 호환성**: `pb.keyframe_insert()`가 레거시 슬롯에서 실패 — F4 IK bake는 FCurve 직접 접근으로 우회
...
- [ ] F9: Auto Map 후 UIList에 매핑/폐기 본이 정확히 표시되는지 확인
- [ ] F9: 사용자 매핑 수정 후 리타게팅 결과에 반영되는지 확인
- [ ] F9: pipeline_runner / batch 경로에서 기존대로 동작하는지 확인
