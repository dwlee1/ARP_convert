...
def generate_bmap_content(analysis, arp_obj=None, ik_legs=False):
    ...
    for role, chain_info in chains.items():
        ...
        if not ctrl_bones:
            ctrl_bones = _dynamic_ctrl_names(role, len(source_bones))
        if not ctrl_bones:
...
def populate_bone_map(analysis, bone_map_collection, arp_obj=None, ik_legs=False):
    """
    분석 결과를 bone_map CollectionProperty에 채움.
    generate_bmap_content()와 동일한 매핑 로직을 사용하되,
    결과를 .bmap 문자열 대신 UI용 CollectionProperty에 저장한다.
    매핑에서 폐기된 소스 본도 is_mapped=False로 포함하여 UI에 표시한다.

    Args:
        analysis: analyze_skeleton() 또는 preview_to_analysis() 결과
        bone_map_collection: bpy CollectionProperty (ARPCONV_BoneMapEntry 타입)
        arp_obj: ARP 아마추어 오브젝트 (있으면 실제 컨트롤러 이름 사용)
        ik_legs: True이면 다리 끝점을 IK 컨트롤러로 매핑
    """
    bone_map_collection.clear()
    ...
    for role, chain_info in chains.items():
        ...
        for src_bone, ctrl_bone in mapping.items():
            entry = bone_map_collection.add()
            entry.source_bone = src_bone
            entry.target_bone = ctrl_bone
            entry.role = role
            entry.is_mapped = True
            entry.is_root = role == "root"
            ik_pole, ik_flag = ik_info.get(ctrl_bone, ("", False))
            entry.ik_enabled = ik_flag
            entry.ik_pole = ik_pole

        # 매핑에서 폐기된 소스 본 추가 (is_mapped=False)
        for src_bone in source_bones:
            if src_bone not in mapped_sources:
                entry = bone_map_collection.add()
                entry.source_bone = src_bone
                entry.target_bone = ""
                entry.role = role
                ...
                entry.is_root = False
                entry.ik_enabled = False
                entry.ik_pole = ""

    # unmapped 커스텀 본 매핑 추가 (원본 이름 유지)
    custom_bones = order_bones_by_hierarchy(
        list(analysis.get("unmapped", [])),
        analysis.get("bone_data", {}),
    )
    for custom_bone in custom_bones:
        entry = bone_map_collection.add()
        entry.source_bone = custom_bone
        entry.target_bone = custom_bone
        entry.role = "unmapped"
        entry.is_mapped = True
        entry.is_root = False
        entry.ik_enabled = False
