    ...
    try:
            ...
            try:
                ...
                if bmap_preset:
                    try:
                        ...
                        result["warnings"].append(".bmap 프리셋 미사용")
                ...
                for i, action_info in enumerate(pre_actions):
                    ...
                    log(
                        f"  [{i + 1}/{len(pre_actions)}] 리타게팅: '{action_name}' ({frame_start}~{frame_end})"
                    )
        ...
        if TEST_FBX_EXPORT:
            ...
            try:
                ...
                fbx_path = (
                    os.path.splitext(blend_path)[0] + "_arp_test.fbx"
                    if blend_path
                    else os.path.join(os.path.expanduser("~"), "_arp_test.fbx")
                )
...
def _test_profile_load():
    ...
    # L 매핑 확인
    assert d2r.get("shoulder_L") == "shoulder_ref.l", (
        f"shoulder_L 매핑 실패: {d2r.get('shoulder_L')}"
    )
    ...
    # R 미러 자동 생성 확인
    assert d2r.get("shoulder_R") == "shoulder_ref.r", (
        f"shoulder_R 미러 실패: {d2r.get('shoulder_R')}"
    )
