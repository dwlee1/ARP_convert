...
def run_analysis(all_bones):
    ...
    face_features = sa.find_head_features(head_name, deform_bones) if head_name else {
...
class TestNeck:
    ...
    def test_neck_contains_expected(self, animal_name, analysis):
        ...
        for bone in expected['neck_contains']:
            assert bone in analysis['neck'], (
                f"neck에 {bone}이 없음. 실제 neck: {analysis['neck']}"
            )
...
class TestLegs:
    ...
    def test_legs_symmetric(self, animal_name, analysis):
        ...
        for prefix in ['back_leg', 'front_leg']:
            ...
            assert l_exists == r_exists, (
                f"{prefix} 비대칭: L={l_exists}, R={r_exists}"
            )
    ...
    def test_ears_symmetric(self, animal_name, analysis):
        ...
        if ear_l or ear_r:
            assert len(ear_l) == len(ear_r), (
                f"귀 비대칭: L={len(ear_l)}본, R={len(ear_r)}본"
            )
